...
def get_patients_for_test(cfg: dict) -> list[dict]:
    ...
    clauses = []
    if days_min:
        clauses.append("(CURRENT_DATE - COALESCE(p.last_logged_date, (p.metadata->>'LastConsultation')::date)) >= %s")
        sql_params.append(int(days_min))
    if days_max:
        clauses.append("(CURRENT_DATE - COALESCE(p.last_logged_date, (p.metadata->>'LastConsultation')::date)) <= %s")
        sql_params.append(int(days_max))
    days_filter = ("AND " + " AND ".join(clauses)) if clauses else ""
    sql_params.append(int(cfg["limit"]))
