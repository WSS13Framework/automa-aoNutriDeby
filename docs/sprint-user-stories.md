...
**Estado (MVP técnico):** worker ``python -m nutrideby.workers.process_kiwify_inbox`` + heurísticas em ``nutrideby.integrations.kiwify_payload`` — ``upsert_patient`` com ``source_system=kiwify`` e ``external_id=order_id``; linhas não reconhecidas marcam ``processed`` com nota ``skip:…``. Validar com payload real da Kiwify e ajustar o parser se as chaves diferirem.
...
## US-06 — Mensagem de voz (áudio) do paciente (backlog)

**Como** paciente em telemetria  
**Quero** enviar **áudio** em vez de só texto  
**Para** registar refeições / notas sem escrever.

**Critérios de aceitação (rascunho — sprint futura)**

1. Canal definido (ex. WhatsApp voice note ou upload na API) com limite técnico: **duração máx.** (ex. 60–120 s) e/ou **tamanho máx.** em MB; rejeitar com mensagem clara se exceder.
2. **STT** (speech-to-text) via serviço acordado (ex. API); idioma por defeito PT; tratamento de erro e retry limitado.
3. Persistência: gravar **transcrição** em `documents` com `doc_type` estável (ex. `telemetry_voice_transcript`) + metadados (`duration_sec`, `source_message_id` se aplicável); política explícita sobre **retenção do ficheiro de áudio** (não guardar / guardar X dias — LGPD).
4. Consentimento e aviso no onboarding (“podes enviar áudio; é convertido em texto”).
5. Teste com fixture de áudio curto (sem PII real) ou ambiente de staging.

**Depende de:** canal WhatsApp (ou equivalente), US-03 padrões de `documents`, orçamento STT.
...
5. **US-06** (áudio + governança) — quando canal e STT estiverem escolhidos
